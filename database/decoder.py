...
def dt_2_epoch(dt_arr, t0=datetime(2000,1,1,0,0,0)):
    ''' convert datetime.datetime to epoch integers '''
...
def epoch_2_dt(ep_arr, t0=datetime(2000,1,1,0,0,0), unit='seconds'):
    ''' convert epoch integers to datetime.datetime '''
...
def insert_msg24(cur, mstr, msg27):
...
def decode_raw_pyais(fpath, conn):
    ...
    batch = {f'msg{i}' : np.ndarray(shape=(0,2)) for i in (1, 2, 3, 5, 18, 19, 24, 27)}
    for msg, idx in zip(msgs, np.array(range(len(msgs)))):
        # filter irrelevant messages
        nearest = msgs[msg4idx[binarysearch(msg4idx, idx)]]
        if msg['type'] not in (1, 2, 3, 5, 11, 18, 19, 24, 27): 
            continue
            ...
            logging.debug(f'discarded msg (bad timestamp): {msg}')
            continue
        else:
