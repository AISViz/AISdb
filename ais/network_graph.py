...
if False:
    ...
    # old code, not used
    # kept for reference while developing new code

    def geofence_old(track_merged, domain, colnames, filters, maxdelta=timedelta(hours=1), **kwargs):
        ...
        for transit_window, in_zones, zoneID in agg_transits_per_segment(track_merged, filters):
            ...
            intersection = np.where(transit_window[:-1,-2] != transit_window[1:,-2])[0] +1
            ...
            if len(intersection) > 2: 
                ...
                zoneset = setrng
                zoneset = np.array([0,0])
                track_stats = segmentinfo(track_static, transit_window, src_zone=zoneID[zoneset], domainname=domain.name)
                track_stats['src_stats'] = zone_stats(track_merged, zoneset)
                track_stats['transit_nodes'] = f"{track_stats['src_zone']}"
                statrows.append(track_stats)
            else:
    ...
    def graph_old(merged, domain, parallel=0,            
                filters=[
