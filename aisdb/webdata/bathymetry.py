...
class Gebco():
    ...
    def __enter__(self):
        ...
        filebounds = lambda fpath: { f[0]: float(f[1:]) 
                for f in fpath.split('gebco_2020_', 1)[1].rsplit('.tif', 1)[0].split('_')
            }

        self.rasterfiles = { f : filebounds(f) for f in 
                { k : None for k in sorted([f for f in os.listdir(data_dir) if f[-4:] == '.tif' and 'gebco' in f ]) }
            }
        ...
        for filepath, bounds in self.rasterfiles.items():
            if bounds['w'] <= lon <=  bounds['e'] and bounds['s'] <= lat <= bounds['n']: 
                if not 'img' in bounds.keys():
                    bounds.update({'img' : Image.open(os.path.join(data_dir, filepath))})
        ...
        for filepath, bounds in self.rasterfiles.items():
            if bounds['w'] <= lon <=  bounds['e'] and bounds['s'] <= lat <= bounds['n']: 
                ...
                depths = np.array([ 
                        ...
                        and (0 <= xlon <= bounds['img'].size[0]) 
                        and (0 <= xlat <= bounds['img'].size[1])
                    ])
