...
def test_write_csv_fromdict(tmpdir):
    ...
    with DBConn(dbpath) as dbconn:
        ...
        aisdb.proc_util.write_csv(
            tracks, fpath=os.path.join(tmpdir, "test_write_csv.csv")
        )
...
def test_getfiledate():
    aisdb.proc_util.getfiledate(
        os.path.join(os.path.dirname(__file__), "testdata", "test_data_20211101.nm4")
    )
    aisdb.proc_util.getfiledate(
        os.path.join(os.path.dirname(__file__), "testdata", "test_data_20210701.csv")
    )
    aisdb.proc_util.getfiledate(
        os.path.join(
            os.path.dirname(__file__), "testdata", "test_data_noaa_20230101.csv"
        ),
        source="NOAA",
    )
