...
def epoch_2_dt(ep_arr, t0=datetime(1970, 1, 1, 0, 0, 0), unit="seconds"):
    ...
    elif isinstance(ep_arr, (float, int, np.uint32, np.int32, np.uint64, np.int64)):
...
def delta_knots(track, rng=None):
    ...
    """
    rng = range(len(track["time"])) if rng is None else rng
    ds = np.array([np.max((1, s)) for s in delta_seconds(track, rng)], dtype=object)
...
def mask_in_radius_2D(tracks, xy, distance_meters):
    ...
    for track in tracks:
        mask = [
            haversine(track["lon"][i], track["lat"][i], xy[0], xy[1]) < distance_meters
...
class Domain:
    ...
    args:
        ...
        zones: list of dictionaries
            ...
            (shapely.geometry.Polygon). MultiPolygon geometries
            will be split into their component polygons, named
            by suffixing the zone name with the part index
    ...
    def _add_zone(self, name, x, y):
        ...
        self.zones.update(
            {name: {"geometry": geom, "maxradius": self._zone_max_radius(geom, x, y)}}
        )
        ...
        for g in self.split_geom(zone):
            if g.centroid.x < -180:
                ...
                if not os.path.isdir(zones_west):
                    os.mkdir(zones_west)
                with open(
                    os.path.join(zones_west, zone["name"] + "_west.txt"), "w"
                ) as w:
                ...
                if not os.path.isdir(zones_east):
                    os.mkdir(zones_east)
                with open(
                    os.path.join(zones_east, zone["name"] + "_east.txt"), "w"
                ) as w:
                    w.writelines(stringify(shiftcoord(x), y))
            else:
                x, y = np.array(g.boundary.coords.xy)
                if not os.path.isdir(zones_corr):
                    os.mkdir(zones_corr)
                with open(
                    os.path.join(zones_corr, zone["name"] + "_corr.txt"), "w"
                ) as w:
                    w.writelines(stringify(x, y))

    @staticmethod
    def _explode_multipolygons(zones):
        """split MultiPolygon zone geometries into component polygon zones"""
        exploded = []
        for zone in zones:
            ...
            if isinstance(geom, MultiPolygon):
                exploded.extend(
                    {"name": f"{zone['name']}_{i}", "geometry": part}
                    for i, part in enumerate(geom.geoms)
                )
            else:
                exploded.append(zone)
        return exploded
    ...
    def __init__(self, name, zones=None):
        ...
        if not zones:
            raise ValueError("domain needs to have atleast one polygon geometry")
        zones = self._explode_multipolygons(zones)
        self.name = name
        self.zones = {}
    ...
    def from_geodataframe(
        cls, gdf, name_column: str | None = None, name: str = "domain"
    ) -> "Domain":
        """build a Domain from a geopandas GeoDataFrame.

        geometries are reprojected to EPSG:4326 when the GeoDataFrame
        CRS differs. multi-part geometries are exploded into their
        component polygons, named by suffixing the zone name with the
        part index.
        ...
        args:
            ...
            name_column (str, optional)
                column containing zone names. row index is used
                when not given
            name (str)
                domain name
        """
        ...
        for idx, row in gdf.iterrows():
            if name_column is not None:
                zone_name = str(row[name_column])
            else:
                zone_name = f"zone_{idx}"
            zones.append({"name": zone_name, "geometry": row.geometry})
    ...
    def point_in_polygon(self, x, y):
        ...
        nearest = {
            k: v
            for k, v in sorted(
                self.nearest_polygons_to_point(x, y).items(), key=lambda item: item[1]
            )
            if v < 0
        }
    ...
    def split_geom(self, zone):
        ...
        """
        merged = shapely.ops.linemerge([zone["geometry"].boundary, self._meridian])
...
class DomainFromPoints(Domain):
    ...
    def __init__(self, points, radial_distances, names=None, domainName="domain"):
