...
wwwpath = os.path.abspath(
    os.path.join(os.path.dirname(os.path.dirname(__file__)), "aisdb_web", "dist_map")
)

wwwpath_visualearth = os.path.abspath(
    os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "aisdb_web", "dist_map_bingmaps"
    )
)


def client_url(
    host: str = "localhost", http_port: int = 3000, visualearth: bool = False
) -> str:
    """construct the URL serving the web client map interface"""
    tag = 2 if visualearth else 1
    return f"http://{host}:{http_port}/index.html?python={tag}&z=2"


def _start_webclient(
    visualearth: bool = False, host: str = "localhost", http_port: int = 3000
):
    if not visualearth:
        path = wwwpath
    else:
    ...
    with socketserver.TCPServer((host, http_port), AISDB_HTML) as httpd:
        try:
            print(f"Serving HTTP assets on {host}:{http_port}")
            httpd.serve_forever()
...
async def _start_webserver(
    ...
    with (
        ...
        if open_browser:
            print(
                "Opening a new browser window to display track data. "
                "Press Ctrl-C to stop the server and close the webpage"
            )
            url = client_url(host, http_port, visualearth)
        ...
        async with websockets.server.serve(fcn, host, ws_port) as server:
            stop = asyncio.Future()
            await stop
            await server
...
def visualize(
    ...
    serves track data via websocket.
    ...
    The host, http_port, and ws_port arguments set the bind address for
    the HTTP asset server and the websocket data server. Defaults serve
    HTTP on localhost:3000 and websocket data on localhost:9924.
    ...
    proc = multiprocessing.Process(
        target=_start_webclient, args=[visualearth, host, http_port]
    )
    proc.start()
    try:
        asyncio.run(
            _start_webserver(
                tracks, domain, visualearth, open_browser, host, http_port, ws_port
            )
        )
        proc.join()
    except KeyboardInterrupt:
