...
def _segment_rng_all(
    ...
    # Time splits
    time_splits = (
        np.nonzero(time_vec[1:] - time_vec[:-1] >= maxtime.total_seconds())[0] + 1
    )

    # Speed splits
    valid_speed_indices = np.nonzero(speed_vec[:] <= maxspeed)[0]
    valid_speed_vec = speed_vec[
        valid_speed_indices
    ]  # Speed values only for valid indices
    ...
    all_splits = np.unique(
        np.concatenate(
            [time_splits, speed_splits, cog_splits, course_splits, distance_splits]
        )
    )
...
def tracks_csv(tracks, skipcols: list | None = None):
    ...
    def _append(track, colnames=colnames, decimals=decimals, track_id=0):
        ...
        for i in range(0, track["time"].size):
            row = [
                (
                    track[c][i]
                    if c in track["dynamic"]
                    else (_sanitize(track[c]) if track[c] != 0 else "")
                )
                for c in colnames
            ]
...
def write_csv(
    ...
    writer = csv.writer(
        f, delimiter=",", quotechar="'", quoting=csv.QUOTE_NONE, dialect="unix"
    )
    for row in tracks_csv(tracks, skipcols=skipcols):
        writer.writerow(row)
...
def glob_files(dirpath, ext=".txt", keyorder=lambda key: key):
    ...
    extfiles = [
        [p[0], sorted([f for f in p[2] if f[-len(ext) :] == ext], key=keyorder)]
        for p in paths
        if len(p[2]) > 0
    ]
...
def getfiledate(filename, source=None):
    ...
    with open(filename, "r") as f:
        ...
        if extension == ".csv":
            ...
            time_format = (
                "%Y-%m-%dT%H:%M:%S" if time_key == "BaseDateTime" else "%Y%m%d_%H%M%S"
            )
            ...
            try:
                ...
                if len(timestamp_str) == 13:  # Check if timestamp is in milliseconds
                    epoch = (
                        epoch // 1000
                    )  # Remove the last 3 digits to convert milliseconds to seconds
                ...
                raise err
            fdate = datetime.fromtimestamp(epoch, tz=timezone.utc).date()
            return fdate
